<template>
  ...
  >
    <slot>
      <div class="swipe-item-inner border-bottom-1px van-hairline--bottom" @click="clickItem">
        <span>{{ item.text }}</span>
      </div>
    </slot>
    <ul class="swipe-btns">
      <li v-for="(btn, i) in btns" ref="btns" :key="i" class="swipe-btn" :style="genBtnStyl(btn)" @click.prevent="clickBtn(btn)">
...
<script>
import { getRect, prefixStyle } from '@/utils/dom'
...
const COMPONENT_NAME = 'swipe-item'
